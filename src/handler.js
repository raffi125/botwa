...
function getSmartTriggerThumb() {
  ...
  try {
    const p = "./assets/images/ScravBot2.jpg";
  ...
  try {
    ...
    if (m.isGroup) {
      ...
      if (isAutoreplyEnabled) {
        let customReplies = [];
        if (Array.isArray(groupData.customReplies)) {
          customReplies.push(...groupData.customReplies);
        } else {
        ...
        for (const replyItem of customReplies) {
          ...
          if (text === replyItem.trigger || (replyItem.trigger.length > 2 && text.includes(replyItem.trigger))) {
      ...
      const chatData = db.getGroup(m.chat) || {};
      const privateAutoreply = chatData.autoreply ?? db.setting("autoreplyPrivate") ?? false;
      console.log(`[DEBUG] Private Chat: m.chat=${m.chat}, chatData.autoreply=${chatData.autoreply}, privateAutoreply=${privateAutoreply}, globalSmartTriggers=${globalSmartTriggers}`);
      if (!privateAutoreply && !globalSmartTriggers) return false;
      isAutoreplyEnabled = privateAutoreply || globalSmartTriggers;

      if (isAutoreplyEnabled) {
        let customReplies = [];
        if (Array.isArray(chatData.customReplies)) {
          customReplies.push(...chatData.customReplies);
        }

        if (chatData.pelayan) {
          try {
            const pelayanMod = await import("../plugins/group/pelayan.js");
            if (pelayanMod.getGuideReplies) {
              customReplies.push(...pelayanMod.getGuideReplies(m.prefix));
            }
          } catch (e) {}
        }
    ...
    if (m.body?.startsWith(">>") && m.isOwner) {
      ...
      try {
        ...
        const execCode = new AsyncFunction(
          ...
          const { generateWAMessage, getBuffer, generateWAMessageFromContent, proto, generateMessageID } = await import('ScravBot')
    ...
    if (!plugin) {
      ...
      if (similarityEnabled) {
        ...
        if (suggestions.length > 0) {
          ...
          await m.reply(
            `❓ Command *${m.command}* tidak ditemukan.\n\nMaksud kamu: *${m.prefix}${suggestions[0]?.command || suggestions[0]}* ?`
          );
        }
      }
