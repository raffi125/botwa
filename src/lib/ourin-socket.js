...
async function extendSocket(sock) {
  ...
  sock.relayMessage = async (jid, message, options = {}) => {
    ...
    return originalRelayMessage(targetJid, message, options);
  };
  ...
  sock.sendStickerPack = async (jid, stickers, m, options = {}) => {
    ...
    const publisher = options.publisher ?? options.author ?? "SCRAVBOT";
  ...
  sock.sendMedia = async function (
    ...
    // Auto-detect image from mime or buffer magic bytes if no type specified
    function isImageMime(mime) {
      return mime && (mime.startsWith("image/") || ["image/jpeg","image/png","image/gif","image/webp"].includes(mime));
    }
    ...
    let payload = {};
    // Resolve effective media type
    let effectiveType = mediaType;
    if (!effectiveType) {
      if (isImageMime(mimeType) || isImageBuffer(data)) effectiveType = "image";
      else if (mimeType && mimeType.startsWith("video/")) effectiveType = "video";
      else if (mimeType && mimeType.startsWith("audio/")) effectiveType = "audio";
    }
  ...
  sock.sendButton = async function (
    ...
    if (!options.footer) msg.footer = config.bot?.name || "SCRAVBOT";
    if (source) {
      let data = source;
  ...
  sock.reply = (jid, text = "", quoted, options = {}) => {
    return Buffer.isBuffer(text)
      ? sock.sendMessage(jid, { image: text, ...options }, { quoted })
      : sock.sendMessage(
          jid,
