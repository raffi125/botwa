...
function supportsInlineAnimation() {
  if (process.env.ScravBot_FORCE_ANIMATION === "true") return true;
  if (process.env.ScravBot_NO_ANIMATION === "true") return false;
...
async function playBootSequence(info = {}) {
  const { name = "SCRAV BOT", version = "1.0.0", mode = "public" } = info;
  console.clear();
  console.log("");
...
function printBanner(mini = false) {
  ...
  console.log("");
  const ascii = figlet.textSync("SCRAV BOT", {
    font: "ANSI Shadow",
