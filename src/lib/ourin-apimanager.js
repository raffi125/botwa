...
/**
 * @typedef {Object} ScravBotApiAuthShape
 * @property {Object<string, any>} [headers]
 * @property {Object<string, any>} [params]
 */

/**
 * @typedef {Object} ScravBotApiProviderShape
 * @property {string} baseURL
 * @property {ScravBotApiAuthShape} [auth]
...
/**
 * @typedef {Object} ScravBotApiRequestOptions
...
/**
 * @typedef {Object} ScravBotApiMultipartFile
 ...
 */
class ScravBotApiProvider {
  /**
   * @param {ScravBotApiManager} mgr
   * @param {string} name
   */
  ...
  /**
   * @param {string} ep
   * @param {ScravBotApiRequestOptions} [opt]
   * @returns {Promise<any>}
   */
  request(ep, opt = {}) {
   ...
   * @param {ScravBotApiMultipartFile[]|Object<string, any>} [files]
   * @param {ScravBotApiRequestOptions} [opt]
   * @returns {Promise<any>}
   */
...
class CovenantApiProvider extends ScravBotApiProvider {
  gpt4(input, opt = {}) {
...
class NeoxrApiProvider extends ScravBotApiProvider {
  whatMusic(input, opt = {}) {
...
class CukiApiProvider extends ScravBotApiProvider {
...
class ApiFaaProvider extends ScravBotApiProvider {
  ...
  nanoBanana(params = {}, opt = {}) {
    return this.get("/faa/nano-banana", params, opt);
  }
...
class ScravBotMainApiProvider extends ScravBotApiProvider {
  ytmp4(input, opt = {}) {
...
class NexrayApiProvider extends ScravBotApiProvider {
  geminiTts(input, opt = {}) {
...
class YupraApiProvider extends ScravBotApiProvider {
  brat(input, opt = {}) {
    return this.get("/api/image/brat", this.asParams(input, "text"), opt);
  }
}

class EmiliabotApiProvider extends ScravBotApiProvider {
...
class AzbryApiProvider extends ScravBotApiProvider {
...
class YuulabsApiProvider extends ScravBotApiProvider {
  tiktok(input, opt = {}) {
    return this.get("/api/downloader/tiktok", this.asParams(input, "url"), opt);
  }
}

class ZenzxzApiProvider extends ScravBotApiProvider {
  youtube(input, opt = {}) {
...
class FgsiApiProvider extends ScravBotApiProvider {
...
class NinerouterApiProvider extends ScravBotApiProvider {
  chat(model, prompt, opt = {}) {
...
/**
 * API manager terpusat untuk provider eksternal ScravBot.
 */
class ScravBotApiManager {
  /**
  ...
  /**
   * @param {string} name
   * @returns {ScravBotApiProviderShape}
   */
  ...
  /**
   * @param {string} name
   * @param {ScravBotApiProviderShape} cfg
   * @returns {ScravBotApiProvider}
   */
  ...
  /**
   * @param {string} name
   * @param {Partial<ScravBotApiProviderShape>} cfg
   * @returns {ScravBotApiProvider}
   */
  ...
  makeProviderClasses() {
    return {
      ...
      ScravBot: ScravBotMainApiProvider,
      ...
      ninerouter: NinerouterApiProvider,
    };
  }
  ...
  makeProviderClient(name) {
    const ProviderClass = this.providerClasses?.[name] || ScravBotApiProvider;
  ...
  makeProviders() {
    return {
      ...
      ninerouter: {
        baseURL: config?.APIkey?.ninerouterBase || "http://192.168.100.59/v1",
        auth: {
          headers: {
            "Authorization": () => {
              const key = config?.APIkey?.ninerouter || "";
...
const ScravBotApi = new ScravBotApiManager();

export { ScravBotApiManager, ScravBotApiProvider, ScravBotApi };
export default ScravBotApi;
