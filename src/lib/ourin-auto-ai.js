...
footer: Powered by Scrav Bot (opsional)
[/RICH:CODE]
...
[RICH:LIST]
title: Info Bot
rows: Nama | Scrav Bot;; Versi | 2.4.5;; Developer | Zann
footer: © Scrav Bot (opsional)
[/RICH:LIST]

5. STICKER (saat user minta sticker, atau untuk ekspresi emosi):
[RICH:STICKER]
url: https://iili.io/BPBdFuj.md.jpg
packname: Scrav Bot (opsional)
  ...
  try {
    ...
    if (rich.type === "STICKER") {
      ...
      await sock.sendImageAsSticker(jid, stickerInput, quoted, {
        packname: packname || config.bot?.name || "Scrav Bot",
    ...
    case "STICKER": {
      ...
      await sock.sendMessage(
        m.chat,
        {
          ...
          packname: config.bot?.name || "ScravBot",
          author: "AutoAI",
        },
        { quoted: m },
      );
