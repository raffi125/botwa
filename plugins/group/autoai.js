...
const pluginConfig = {
  ...
  usage:
    ".autoai on/off --ScravBotmode=<character|custom> --logic=<custom instruction> --type=<text|voice>",
  example: ".autoai on --ScravBotmode=furina --type=voice",
  isOwner: false,
  isPremium: false,
  isGroup: true,
  isPrivate: false,
...
async function handler(m) {
  ...
  if (subcmd === "listpersona") {
    ...
    txt += `> .autoai on --ScravBotmode=<key>\n`;
  ...
  if (subcmd === "global") {
    ...
    if (globalMode === "on") {
      const modeMatch = fullArgs.match(/--ScravBotmode=(\w+)/i);
      const typeMatch = fullArgs.match(/--type=(text|voice)/i);
      const logicMatch = fullArgs.match(
        /--logic=(.+?)(?=\s+--(?:ScravBotmode|type|logic)|$)/i,
      );
      ...
      if (charKey === "custom" && customLogic) {
        ...
        if (existingGlobal.instruction) {
          ...
          return m.reply(
            `❌ Belum ada persona global yang diset!\n\n> .autoai global on --ScravBotmode=furina\n> .autoai global on --ScravBotmode=custom --logic=...`,
          );
        }
      } else {
        const charList = [
  ...
  if (!mode || !["on", "off"].includes(mode)) {
    ...
    txt += `*Penggunaan:*\n`;
    txt += `> .autoai on --ScravBotmode=<karakter|custom> --type=<text|voice>\n`;
    txt += `> .autoai off\n`;
    txt += `> .autoai tambahpersona nama | logic\n`;
    txt += `> .autoai hapuspersona nama\n`;
    ...
    txt += `*Contoh:*\n`;
    txt += `> .autoai on --ScravBotmode=furina --type=text\n`;
    txt += `> .autoai on --ScravBotmode=custom --logic=kamu adalah nexa ai\n`;
    txt += `> .autoai tambahpersona nexa | kamu adalah nexa ai\n`;
    txt += `> .autoai global on --ScravBotmode=furina`;
    return m.reply(txt);
  }
  ...
  if (!charKey) {
    ...
    return m.reply(
      `❌ Karakter tidak valid!\n\n> Karakter tersedia: ${charList}\n\n> Contoh: .autoai on --ScravBotmode=furina --type=voice\n> Custom: .autoai on --ScravBotmode=custom --logic=kamu adalah nexa ai`,
    );
  }
  ...
  if (charKey === "custom") {
    if (!customLogic) {
      return m.reply(
        `❌ Mode custom membutuhkan --logic!\n\n> Contoh: .autoai on --ScravBotmode=custom --logic=kamu adalah nexa ai, ...`,
      );
    }
