...
function getContextInfo(title = '?? ???', body = 'Jasa Pesan Massal') {
...
async function handler(m, { sock }) {
    ...
    try {
        ...
        await sock.sendMessage(m.chat, {
            ...
            contextInfo: getContextInfo('?? ???', 'Sending...')
        }, { quoted: m })
        ...
        const contextInfo = getContextInfo('?? ???', config.bot?.name || 'ScravBot')
            ...
            if (global.stopjpm) {
                ...
                await sock.sendMessage(m.chat, {
                    text: `?? *??? ??????????*\n\n` +
                        ...
                        `? ? ?????s??: \`${successCount}\`\n` +
                        `? ? ?????: \`${failedCount}\`\n` +
                        `? ?? s?s?: \`${groupIds.length - successCount - failedCount}\`\n` +
                        `????`,
        ...
        await sock.sendMessage(m.chat, {
            text: `? *??? s???s??*\n\n` +
                ...
                `? ?? ?????: \`${groupIds.length}\`\n` +
                `????`,
            contextInfo: getContextInfo('? s???s??', `${successCount}/${groupIds.length}`)
        }, { quoted: m })
