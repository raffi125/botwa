...
async function handler(m, { sock }) {
  ...
  if (!trxId) {
    return m.reply(
      `? *KONFIRMASI TRANSAKSI*\n\n` +
        ...
        `?? *Cara penggunaan:*\n` +
        `1?? Reply pesan dari pembeli (yang sudah membayar ??)\n` +
        `2?? Ketik \`${m.prefix}done TRX-001\`\n\n` +
        `?? Bot akan otomatis:\n` +
        ...
        `?? _Pastikan Anda sudah menerima bukti pembayaran sebelum konfirmasi_ ??`,
    );
  }
  ...
  invoiceTxt += `??? Tipe: *${typeLabel}*\n`;
  invoiceTxt += `?? Harga: *${formatPrice(trx.price)}*\n\n`;

  if (stockItemDetail) {
    invoiceTxt += `?? *Data Produk:*\n\`\`\`\n${stockItemDetail}\n\`\`\`\n\n`;
    invoiceTxt += `?? _Simpan data di atas dengan baik. Jangan bagikan ke siapapun_ ??\n\n`;
  ...
  if (product) {
    const stockDisplay =
      product.type === "fisik"
        ? `${product.stock === -1 ? "?? Unlimited" : product.stock + " pcs"}`
