 ...
 * Enhanced for ScravBotAI with visual board and full contextInfo
 */
  ...
  const commands = {
      ...
      await sock.sendMessage(
        m.chat,
        {
          text:
            ...
            `? ?? \`${prefix}ut start\` - Mulai\n` +
            `? ?? \`${prefix}ut info\` - Info room\n` +
    ...
    exit: async () => {
      ...
      if (Object.keys(ut[m.chat].players).length === 0) {
        delete ut[m.chat];
        return m.reply(`??? Room dihapus karena tidak ada player.`);
      }
    ...
    delete: async () => {
      ...
      delete ut[m.chat];
      await m.react("???");
      await m.reply(`??? Room berhasil dihapus!`);
    },
  };
