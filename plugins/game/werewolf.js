...
const ROLES = {
    ...
    guardian: { emoji: '???', name: 'Guardian', team: 'village', desc: 'Lindungi player tiap malam' },
    ...
    const commands = {
        ...
        start: async () => {
            ...
            await m.reply(`?? *GAME DIMULAI!*\n\n` +
                ...
                `?? Werewolf berburu...\n` +
                `?? Waktu malam: ${PHASE_DURATION.night / 1000} detik`,
                { mentions: ww[m.chat].players.map(p => p.id) })
        ...
        player: async () => {
            ...
            const playerList = ww[m.chat].players.map((p, i) => {
                const status = p.alive ? '?' : `?? (${ROLES[p.role]?.name || 'Unknown'})`
                return `${p.number}. @${p.id.split('@')[0]} ${status}`
            }).join('\n')
            
            const phaseEmoji = ww[m.chat].phase === 'night' ? '??' : ww[m.chat].phase === 'day' ? '??' : '??'
        ...
        exit: async () => {
            ...
            if (ww[m.chat].players.length === 0) {
                if (ww[m.chat].timeout) clearTimeout(ww[m.chat].timeout)
                delete ww[m.chat]
                return m.reply(`??? Room dihapus karena kosong.`)
            }
...
async function processNightActions(chatId, sock, db, prefix) {
    ...
    let nightReport = `?? *PAGI HARI KE-${ww[chatId].day}*\n\n`
    ...
    if (winner) {
        await sock.sendMessage(chatId, {
            ...
            contextInfo: getWWContextInfo('?? DAY', 'Pagi telah tiba...', thumbDay, ww[chatId].players.map(p => p.id))
        })
    ...
    nightReport += `??????????\n\n`
    nightReport += `> ??? Waktunya voting!\n`
    nightReport += `> Ketik \`${prefix}ww vote <nomor>\`\n`
    nightReport += `> ?? Waktu: ${PHASE_DURATION.day / 1000} detik`
    
    await sock.sendMessage(chatId, {
        text: nightReport,
        mentions: ww[chatId].players.map(p => p.id),
        contextInfo: getWWContextInfo('?? DAY', 'Voting time!', thumbDay, ww[chatId].players.map(p => p.id))
    })
...
async function executeVote(chatId, sock, db, prefix) {
    ...
    if (isTie || maxVotes === 0) {
        ...
        if (player) {
            ...
            resultText += `?? @${eliminated.split('@')[0]} dieliminasi!\n`
    ...
    if (winner) {
        await sock.sendMessage(chatId, {
            ...
            contextInfo: getWWContextInfo('?? VOTING', 'Hasil voting', thumbDay)
        })
        await endGame(chatId, sock, db, winner)
        return
    }
    ...
    resultText += `> ?? Waktu: ${PHASE_DURATION.night / 1000} detik`
