...
const pluginConfig = {
  name: "ScravBotbanana2",
  alias: [],
  category: "ai",
  description: "Buat gambar dengan AI menggunakan prompt",
  usage: ".ScravBotbanana2 <prompt>",
  example: ".ScravBotbanana2 make it anime style",
  isOwner: false,
  isPremium: false,
  isGroup: false,
  isPrivate: false,
...
async function handler(m, { sock }) {
  ...
  if (!prompt) {
    return m.reply(
      `🍌 *ScravBot BANANA SUPER 2*\n\n` +
        `> Buat gambar dengan AI\n\n` +
        `\`Contoh: ${m.prefix}ScravBotbanana2 make a cat\``,
    );
  }
